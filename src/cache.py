...
def diff(old_state: dict, new_state: dict, series_id: str) -> UpdateResult:
    ...
    added = [(ep_id, ep_title, is_free) for ep_id, ep_title, is_free in new_episodes if ep_id not in old_ids]
    free_episodes = {ep_id for ep_id, _, is_free in old_episodes if is_free}
    new_unlocked = [ep_title for ep_id, ep_title, is_free in new_episodes if ep_id not in free_episodes and is_free]
...
def _filter_apollo(apollo: dict, series_id: str) -> dict:
    ...
    return {
        key: value for key, value in apollo.items()
